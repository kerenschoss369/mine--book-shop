...
function getNextBookId(bookId) {
    ...
    if (idx === gBooks.length - 1) idx = -1;
    const nextBook = gBooks[idx + 1];
    const nextBookId = nextBook.id;
    return Promise.resolve(nextBookId);
}

function getPrevBookId(bookId) {
    var idx = gBooks.findIndex((book) => book.id === bookId);
    if (idx === 0) idx = gBooks.length;
    const prevBook = gBooks[idx - 1];
    const prevBookId = prevBook.id;
    return Promise.resolve(prevBookId);
}
...
function addGoogleBook(googleBook) {
    ...
    const newBook = {
        ...
        "listPrice": {
            "amount": 0,
            "currencyCode": googleBook.saleInfo.saleability,
            "isOnSale": false
        }
    }
